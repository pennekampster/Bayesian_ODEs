...
model <- cmdstan_model("FRT_mixed.stan")
n_chains <- 3
# if(!dir.exists("Draws")) dir.create("Draws")
fit <- model$sample(data = getData(D, temp = NULL),
                       init = replicate(n_chains, list(b_log = -4.106669, 
                                                       c_log = -5.600194, 
                                                       h_log = -3.531813, 
                                                       K_log = 7.901859,
                                                       q = 0.867401,
                       ...
                       iter_warmup = 300, iter_sampling = 500, chains = n_chains, parallel_chains = n_chains, output_dir = "Draws", output_basename = "fit_poly", seed = 1)
